const countries = Object.entries(gameData);
...
function drop(e) {
  e.preventDefault();
  const droppableSquareData = e.target.getAttribute('id');
  const draggableFlagData = e.dataTransfer.getData('text/plain');
  
  if (draggableFlagData === droppableSquareData && secondsRemaining !== 0) {
   e.target.innerText = '';
   e.target.style.border = '3px solid green';
   
   e.target.appendChild(document.getElementById(draggableFlagData));
   correctAnswers++;
   ...
   const droppedImage = document.querySelector(`#${draggableFlagData} >img`);
   droppedImage.setAttribute('draggable', 'false');
  }
  else if (draggableFlagData !== droppableSquareData && secondsRemaining !== 0) {
